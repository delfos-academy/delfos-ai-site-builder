...
**Objetivo.** Gerar o plano mestre **token-eficiente**: um `PLAN.md` enxuto (índice + critério de pronto por milestone) e N arquivos `MILESTONES/NN-*.md` com as tasks detalhadas. Cada um é carregado **só quando for trabalhado**.

**Entradas.** `BRIEF.md`, `DESIGN_SYSTEM.md`, `ARCHITECTURE.md`.

**Saídas.**
- `PLAN.md` no projeto-alvo (template em [templates/PLAN.md.tpl](../templates/PLAN.md.tpl)).
- `MILESTONES/NN-<slug>.md` para cada macro-etapa (template em [templates/MILESTONE.md.tpl](../templates/MILESTONE.md.tpl)).
- `WORKFLOW.md` com regras de branch/PR/merge (template em [templates/WORKFLOW.md.tpl](../templates/WORKFLOW.md.tpl)).
...
Separação:
- **`PLAN.md` (curto, ≤ 200 linhas)**: visão geral + lista de milestones + critério de "pronto" de cada um (1-2 linhas). Sempre carregado.
- **`MILESTONES/NN-*.md`**: tasks completas, decisões da fase, status. Carregado pelo Claude **só quando entrar nesse milestone**.
...
```
00-bootstrap            Setup repo, DB schema, auth, CI/CD, layout base
...
|---|---|---|---|
| 00 | Bootstrap | [MILESTONES/00-bootstrap.md] | `pnpm build` ok, login funciona, deploy preview ativo |
| 01 | … | … | … |
...
### 5. Mostrar `PLAN.md` para o user e aguardar OK
...
Após aprovação, gerar os arquivos `MILESTONES/NN-*.md`.
...
- [ ] Cada `MILESTONES/NN-*.md` tem ≥ 3 testes obrigatórios
